...
- **No live-eval entry point yet** — the deterministic runner is complete and the roster is written to be replayable against a live model, but the command that boots the MedHarness profile, runs a subset against the configured model, and writes a report is not built. That is the next step, not a redesign.
